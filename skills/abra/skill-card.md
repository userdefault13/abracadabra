...
- `ABRA_KEY` stays in env and is sent from an in-process HTTP client — never on a command line. Fetched secrets go into a **minimal** child process env (not a full `process.env` clone) and die with that process — **never** write them to disk (repo, cwd, or home-dir dumps).
...
- Env injection only. If a runtime cannot take environment variables, stop and tell the human — do not invent a plaintext file fallback.
