...
Local secrets vault for agents (`abra serve` on `127.0.0.1:7331`). Fetch env vars,
API tokens, and wallet keys **by name** via scoped API key or MCP — never paste
secrets in chat. Optional Base USDC treasury spends via Touch ID
(`request_treasury_payment` / `abra treasury pay`).
...
- Prefer scoped `ABRA_KEY` + `POST /secret` for unattended work; MCP `get_secrets` when a human is present.
